...
object PhoneNumberUtils {
    val context = LoungeApplication.getInstance()

    private val DEFAULT_COUNTRY_CODE_INT = 1
    ...
    fun getPseudoValidPhoneNumber(phoneNumber: String, countryCode: String?): String? {
        ...
        return if (countryCode == null) {
            PhoneNumberUtils.formatUsingCurrentCountry(phoneNumber, context)
        } else {
     ...
     */
    @Nullable
    fun getCountryCodeForPhoneNumber(normalizedPhoneNumber: String): String? {
    ...
    fun getCountryList(context: Context): ArrayList<String> {
        ...
        for (i in 0 until codes.size()) {
            ...
            for (j in innerList.indices) {
                ...
                val finalString: String = "" + key + "  " + "(${locale.displayCountry})"
    ...
    fun isPhoneNumberValid(phoneNumber: String?, countryCode: String?): Boolean {
        ...
        try {
            val regionCode = countryCode?.let { getRegionCodeFromCountryCode(it) }
    ...
    private fun getRegionCodeFromCountryCode(countryCode: String): String? {
        val array = createCountryCodeToRegionCodeMap()
        try {
            return array.get(countryCode.toInt())?.first()
        }catch (e: NumberFormatException){
            return array.get(91)?.first()
        }
    }
}
